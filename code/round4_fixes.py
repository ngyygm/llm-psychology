...
def run_synthetic_baselines(all_results):
    ...
    pir_csv = RESULTS_DIR / "pir_by_model_domain.csv"
    if pir_csv.exists():
    ...
    sim_df.to_csv(RESULTS_DIR / "synthetic_baselines.csv", index=False)
    return sim_df
...
def run_leave_one_persona_out(all_results):
    ...
    loo_df.to_csv(RESULTS_DIR / "leave_one_persona_out.csv", index=False)
    return loo_df
...
def main():
    ...
    print(f"\nAll outputs:")
    print(f"  results/synthetic_baselines.csv")
    print(f"  results/leave_one_persona_out.csv")
