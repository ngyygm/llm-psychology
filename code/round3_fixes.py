...
def generate_human_benchmarks():
    ...
    alpha_csv = RESULTS_DIR / "cronbach_alpha_by_domain.csv"
    llm_alpha_map = {}
    if alpha_csv.exists():
    ...
    conv_csv = RESULTS_DIR / "convergent_validity_enhanced.csv"
    conv_map = {}
    if conv_csv.exists():
    ...
    fr_csv = RESULTS_DIR / "forward_reverse_correlation.csv"
    fr_corr = "N/A"
    if fr_csv.exists():
    ...
    cong_csv = RESULTS_DIR / "item_level_congruence.csv"
    max_phi = "N/A"
    if cong_csv.exists():
    ...
    pd.DataFrame(rows).to_csv(RESULTS_DIR / "human_vs_llm_benchmarks.csv", index=False)
    return rows
...
def run_leave_one_out(all_results):
    ...
    loo_df.to_csv(RESULTS_DIR / "leave_one_out_robustness.csv", index=False)
    return loo_df
...
def main():
    ...
    print("=" * 70)
    print(f"\nNew outputs:")
    print(f"  results/human_vs_llm_benchmarks.csv")
    print(f"  results/leave_one_out_robustness.csv")
