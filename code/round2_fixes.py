...
def run_item_level_cfa(all_results):
    ...
    # Save
    pd.DataFrame(alpha_results).to_csv(RESULTS_DIR / "cronbach_alpha_by_domain.csv", index=False)
    pd.DataFrame(congruence, index=[f"F{i+1}" for i in range(5)],
                 columns=domains_order).to_csv(RESULTS_DIR / "item_level_congruence.csv")
...
def run_safety_item_analysis(all_results):
    ...
    # Save
    safety_df.to_csv(RESULTS_DIR / "safety_item_analysis.csv", index=False)
    return safety_df
...
def run_bootstrap_ci(all_results):
    ...
    conv_df = pd.read_csv(RESULTS_DIR / "convergent_validity_enhanced.csv")
    ...
    ]).to_csv(RESULTS_DIR / "bootstrap_ci_results.csv", index=False)
...
def main():
    ...
    print("=" * 70)
    print(f"\nNew outputs:")
    print(f"  results/cronbach_alpha_by_domain.csv")
    print(f"  results/item_level_congruence.csv")
    print(f"  results/safety_item_analysis.csv")
    print(f"  results/bootstrap_ci_results.csv")
    print(f"  results/acquiescence_mechanism.csv")
