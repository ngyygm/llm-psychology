...
FIG_DIR = CODE_DIR.parent / "paper" / "figures"
...
def load_json_results() -> tuple[dict[str, dict], list[str], list[str], list[str], np.ndarray]:
    data: dict[str, dict] = {}
    for path in sorted(DATA_DIR.glob("exp_mbti_*.json")):
...
def fig_factor_structure() -> None:
    ...
    load = pd.read_csv(RESULTS_DIR / "efa_domain_loadings.csv", index_col=0)
...
def fig_cronbach() -> None:
    alpha = pd.read_csv(RESULTS_DIR / "cronbach_alpha_by_domain.csv")
...
def fig_pir_sdr() -> None:
    pir = pd.read_csv(RESULTS_DIR / "pir_by_model_domain.csv")
    sdr = pd.read_csv(RESULTS_DIR / "pir_sdr_crossvalidation.csv")
...
def fig_acquiescence() -> None:
    pir = pd.read_csv(RESULTS_DIR / "pir_by_model_domain.csv")
    acq = pd.read_csv(RESULTS_DIR / "acquiescence_mechanism.csv")
    domain = (
...
def fig_variance() -> None:
    var = pd.read_csv(RESULTS_DIR / "variance_decomposition.csv")
...
def fig_measurement_invariance() -> None:
    inv = pd.read_csv(RESULTS_DIR / "persona_invariance.csv")
...
def fig_response_styles() -> None:
    styles = pd.read_csv(RESULTS_DIR / "response_styles.csv")
    default = styles[styles["persona"] == "Default"].copy()
    pir = pd.read_csv(RESULTS_DIR / "pir_sdr_crossvalidation.csv")
    lie = pd.read_csv(RESULTS_DIR / "sdr_by_model.csv")[["model", "lie_scale"]]
...
def fig_convergent_validity() -> None:
    conv = pd.read_csv(RESULTS_DIR / "convergent_validity_enhanced.csv")
...
def fig_invariance_robustness() -> None:
    inv = pd.read_csv(RESULTS_DIR / "persona_invariance.csv")
    loo_model = pd.read_csv(RESULTS_DIR / "leave_one_out_robustness.csv")
    loo_persona = pd.read_csv(RESULTS_DIR / "leave_one_persona_out.csv")
...
def fig_factor_collapse() -> None:
    congr = pd.read_csv(RESULTS_DIR / "item_level_congruence.csv", index_col=0)
    eig = pd.read_csv(RESULTS_DIR / "efa_eigenvalues.csv")
...
def fig_model_dashboard() -> None:
    pir = pd.read_csv(RESULTS_DIR / "pir_sdr_crossvalidation.csv").set_index("model")
    sdr = pd.read_csv(RESULTS_DIR / "sdr_by_model.csv").set_index("model")
    inv = pd.read_csv(RESULTS_DIR / "persona_invariance.csv").groupby("model")["pearson_r"].mean()
    df = pd.DataFrame(
        {
            "PIR": pir["mean_pir"],
