...
ITEMS_PATH = DATA_DIR / "items_battery.json"
...
def load_results() -> dict[str, dict]:
    files = sorted(DATA_DIR.glob("exp_mbti_*.json"))
    if not files:
...
def plot_robustness(robustness_df: pd.DataFrame) -> None:
    ...
    metric_labels = ["z-Euclidean", "Cosine", "Mahalanobis"]
    ...
    for i, metric in enumerate(metrics):
        ...
        for bar in bars:
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, h + 0.015,
                    f"{h:.3f}", ha="center", va="bottom", fontsize=5.2,
                    fontweight="bold", color=colors[i])
    ax.set_xticks(x)
    ax.set_xticklabels(["All scales", "Likert only", "Binary only", "IPIP only"])
    ax.set_ylim(0.85, 1.02)
    ax.set_ylabel("Top-1 fidelity")
    ax.set_title("Persona-fidelity robustness across metrics and scale subsets", loc="left", pad=5, fontweight="bold")
    ax.legend(fontsize=6.2, frameon=False, ncol=3, loc="lower left",
