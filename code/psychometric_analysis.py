...
def load_all_results():
    results = {}
    for fpath in sorted(DATA_DIR.glob("exp_mbti_*.json")):
...
def run_efa_analysis(all_results):
    ...
    # Save
    loadings_df.to_csv(RESULTS_DIR / "efa_domain_loadings.csv")
    pd.DataFrame({"domain": domain_names, "eigenvalue": eigenvalues}).to_csv(
        RESULTS_DIR / "efa_eigenvalues.csv", index=False)
    if len(model_factor_df) > 0:
        model_factor_df.to_csv(RESULTS_DIR / "efa_per_model_congruence.csv", index=False)
...
def run_pir_sdr_analysis(all_results):
    ...
    # Save
    pir_df.to_csv(RESULTS_DIR / "pir_by_model_domain.csv", index=False)
    sdr_df.to_csv(RESULTS_DIR / "sdr_by_model.csv", index=False)
    pir_sdr.to_csv(RESULTS_DIR / "pir_sdr_crossvalidation.csv", index=False)
    persona_pir_df.to_csv(RESULTS_DIR / "pir_by_persona.csv", index=False)
...
def run_dif_analysis(all_results):
    ...
    # Save
    item_var.to_csv(RESULTS_DIR / "item_variance_analysis.csv", index=False)
    dif_df.to_csv(RESULTS_DIR / "dif_model_family.csv", index=False)
    conv_df.to_csv(RESULTS_DIR / "convergent_validity_enhanced.csv", index=False)
    if safety_items:
        pd.DataFrame(safety_items).to_csv(RESULTS_DIR / "safety_contaminated_items.csv", index=False)
...
def run_response_style_analysis(all_results):
    ...
    rs_df.to_csv(RESULTS_DIR / "response_styles.csv", index=False)
    return rs_df
...
def run_measurement_invariance(all_results):
    ...
    inv_df.to_csv(RESULTS_DIR / "persona_invariance.csv", index=False)
    model_inv.to_csv(RESULTS_DIR / "persona_invariance_by_model.csv", index=False)
...
def generate_integrated_report(all_results, efa_results, pir_results, var_results,
    ...
    report = "\n".join(lines)
    with open(RESULTS_DIR / "psychometric_validation_report.txt", "w") as f:
        f.write(report)
    print(report)
    return report
