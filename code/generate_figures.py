...
FIGURES_DIR = CODE_DIR.parent / "paper" / "figures"
...
def save(fig, name):
    fig.savefig(FIGURES_DIR / name, dpi=300, bbox_inches='tight', pad_inches=0.05)
    plt.close(fig)
    print(f"  Saved {name}")
...
def fig1_factor_structure():
    eig_df = pd.read_csv(RESULTS_DIR / "efa_eigenvalues.csv")
    load_df = pd.read_csv(RESULTS_DIR / "efa_domain_loadings.csv", index_col=0)
...
def fig2_cronbach_comparison():
    bench = pd.read_csv(RESULTS_DIR / "human_vs_llm_benchmarks.csv")
    synth = pd.read_csv(RESULTS_DIR / "synthetic_baselines.csv")
...
def fig3_acquiescence_mechanism():
    pir_df = pd.read_csv(RESULTS_DIR / "pir_by_model_domain.csv")
    acq_df = pd.read_csv(RESULTS_DIR / "acquiescence_mechanism.csv")
...
def fig4_variance_decomposition():
    ...
    boot_df = pd.read_csv(RESULTS_DIR / "bootstrap_ci_results.csv")
...
def fig6_invariance_robustness():
    inv_df = pd.read_csv(RESULTS_DIR / "persona_invariance.csv")
