#!/usr/bin/env python3
"""
Analysis Script for Vendor Experiment (V3.1)
Study 1: Cross-vendor OLR + OLS ANOVA + FDR correction
Study 2: Within-vendor trajectory analysis (descriptive)
...
def get_study1_data(df: pd.DataFrame) -> pd.DataFrame:
    """Filter Study 1 data (cross-vendor comparison)."""
    # Study 1 = study==1 AND thinking_mode=="chat"
...
def get_study2_data(df: pd.DataFrame) -> pd.DataFrame:
    """Filter Study 2 data (within-vendor evolution)."""
...
def reliability_analysis(df: pd.DataFrame, output_dir: str = "results/vendor_exp"):
    ...
    for model_id in sorted(df["model"].unique()):
        ...
        vendor = model_data.iloc[0].get("vendor", model_id.split("/")[0])
        ...
        for dim in DIMENSIONS:
            ...
            if len(dim_scores) < 2:
                results.append({
                    "model": model_id, "vendor": vendor, "dimension": dim,
...
def study1_ols_anova(df: pd.DataFrame):
    """OLS ANOVA for Study 1: Vendor -> dimension means.
    Primary analysis uses 8 dimensions (α > 0.70). HEXACO-H excluded (α=0.27).
    """
    print("\n" + "=" * 80)
    print("STUDY 1: OLS ANOVA (Vendor -> Dimension Means)")
    print("=" * 80)
    ...
    for dim in DIMENSIONS:
        ...
        groups = [df[df["vendor"] == v][dim].dropna().values for v in vendors]

        # Filter out vendors with < 2 observations
        valid_groups = [(v, g) for v, g in zip(vendors, groups) if len(g) >= 2]
        if len(valid_groups) < 2:
            print(f"\n  {DIM_LABELS[dim]}: insufficient data")
            continue

        vendors_valid = [v for v, g in valid_groups]
        ...
        partial_eta_sq = ss_between / (ss_between + ss_total) if (ss_between + ss_total) > 0 else 0

        # Kruskal-Wallis (non-parametric robustness)
        h_stat, kw_p = stats.kruskal(*groups_valid)

        # Descriptive stats per vendor
        vendor_means = {v: {"mean": np.mean(g), "sd": np.std(g, ddof=1), "n": len(g)}
                       for v, g in zip(vendors_valid, groups_valid)}

        results.append({
            "dimension": dim,
            "label": DIM_LABELS[dim],
            "n_vendors": len(vendors_valid),
            ...
            "KW_H": round(h_stat, 3),
            "KW_p": round(kw_p, 6),
            "vendor_means": vendor_means,
        })

        print(f"\n  {DIM_LABELS[dim]:25s}: F({len(vendors_valid)-1},{len(all_data)-len(vendors_valid)}) = {f_stat:.2f}, "
              f"p = {p_value:.2e}, η²p = {partial_eta_sq:.4f}")
        print(f"    Kruskal-Wallis H = {h_stat:.2f}, p = {kw_p:.2e}")
        for v in vendors_valid:
            vm = vendor_means[v]
...
def study1_olr(df: pd.DataFrame):
    ...
    for dim in DIMENSIONS:
        ...
        obs_df = df[["vendor", "arch", dim]].dropna().copy()
        ...
        # Create vendor dummies (first vendor as reference)
        vendors = sorted(obs_df["vendor"].unique())
        ref_vendor = vendors[0]

        # Architecture as covariate
        obs_df["arch_dense"] = (obs_df["arch"] == "Dense").astype(int)

        try:
            # OLR with vendor + architecture
            X = pd.get_dummies(obs_df["vendor"], drop_first=True, dtype=float)
            X["arch_dense"] = obs_df["arch_dense"].values
            ...
            # Wald test for overall vendor effect
            # Pseudo R-squared
            ...
            # Coefficients for each vendor
            vendor_coefs = {}
            for v in vendors[1:]:
                col = f"vendor[T.{v}]" if f"vendor[T.{v}]" in result.params.index else v
    ...
    if olr_results:
        ...
        for r in olr_results:
            for v, coef in r["vendor_coefs"].items():
...
def study1_cohen_d(df: pd.DataFrame):
    """Compute pairwise Cohen's d between vendors for each dimension.
    ...
    - 0.5 ≤ d < 0.8: medium — substantial vendor-specific response tendency
    - d ≥ 0.8: large — strong vendor signature, likely reflects systematic differences
    ...
    for dim in DIMENSIONS:
        ...
        for i in range(n_vendors):
            for j in range(i + 1, n_vendors):
                v1, v2 = vendors[i], vendors[j]
                g1 = df[df["vendor"] == v1][dim].dropna().values
                g2 = df[df["vendor"] == v2][dim].dropna().values
...
def study2_descriptive(df: pd.DataFrame):
    """Descriptive analysis of within-vendor evolution trajectories."""
    print("\n" + "=" * 80)
    print("STUDY 2: Within-Vendor Trajectories (Descriptive)")
    print("=" * 80)

    results = []

    for vendor in sorted(df["vendor"].unique()):
        if not vendor:  # skip records with empty vendor
            continue
        vendor_data = df[df["vendor"] == vendor]
        subgroups = vendor_data["subgroup"].fillna("").unique()
        subgroups = [s for s in subgroups if s and s != ""]  # filter empty

        if not subgroups:
            print(f"\n  {vendor}: No subgroup data (Study 2)")
            continue

        print(f"\n  {vendor}:")

        for sg in sorted(subgroups):
            sg_data = vendor_data[vendor_data["subgroup"].fillna("") == sg]
            if len(sg_data) == 0:
                continue
...
def floor_effect_robustness(df: pd.DataFrame):
    ...
    for dim in DIMENSIONS:
        ...
        vendors = sorted(df_clean["vendor"].unique())
        groups = [df_clean[df_clean["vendor"] == v][dim].dropna().values for v in vendors]
        valid_groups = [(v, g) for v, g in zip(vendors, groups) if len(g) >= 2]
        ...
        print(f"    Remaining: {len(df_clean)} obs, {len(vendors_valid)} vendors")
...
def hexaco_descriptive_observation(df: pd.DataFrame):
    ...
    for m, v in floor_models.items():
        vendor = df[df["model"] == m]["vendor"].iloc[0] if len(df[df["model"] == m]) > 0 else "?"
        print(f"    {m.split('/')[-1]:35s}: H={v:.2f} ({vendor})")
    ...
    if len(non_floor_models) >= 3:
        ...
        if len(s1) > 0:
            vendors_s1 = sorted(s1["vendor"].unique())
            groups = [s1[s1["vendor"] == v][HEXACO_DIM].dropna().values for v in vendors_s1]
            valid = [(v, g) for v, g in zip(vendors_s1, groups) if len(g) >= 2]
            if len(valid) >= 2:
                h_stat, kw_p = stats.kruskal(*[g for _, g in valid])
                print(f"\n  Kruskal-Wallis on non-floor Study 1 models: H={h_stat:.2f}, p={kw_p:.2e}")
                print(f"  → Vendor differences in HEXACO-H persist even after removing floor-effected models")
...
def alignment_artifact_analysis(df: pd.DataFrame):
    ...
    A dimension is classified as an ALIGNMENT ARTIFACT if it shows low inter-vendor
    variance (SD < threshold), meaning all vendors respond similarly regardless of their
    different training procedures. Such dimensions cannot distinguish vendors.

    IMPORTANT: A dimension can be alignment-SENSITIVE (responds to RLHF) AND still show
    high inter-vendor variance. Alignment sensitivity ≠ alignment artifact. For example,
    HEXACO-H (morality) is alignment-sensitive but may show high cross-vendor variance if
    different vendors apply different alignment strengths.

    Classification rule (data-driven):
    - ALIGNMENT ARTIFACT: inter-vendor SD < 0.15 (near-zero cross-vendor variation)
    - DISCRIMINATIVE: inter-vendor SD >= 0.15 (sufficient variation to distinguish vendors)
    """
    print("\n" + "=" * 80)
    print("ALIGNMENT ARTIFACT ANALYSIS")
    print("=" * 80)
    print("  Identifying dimensions with near-zero inter-vendor variance (alignment artifacts)")
    print("  These dimensions cannot distinguish vendors and are excluded from discriminative analysis")
    print("  Classification is DATA-DRIVEN (inter-vendor SD), not theory-driven")
    ...
    for dim in DIMENSIONS:
        ...
        # Classification: data-driven based on inter-vendor SD
        low_variance = sd_val < 0.15  # Near-zero cross-vendor variation
...
def power_analysis(df: pd.DataFrame):
    """Compute power analysis for Study 1 cross-vendor comparisons.

    Estimates minimum detectable Cohen's d for pairwise vendor comparisons
    ...
    vendors = s1["vendor"].unique()
    n_vendors = len(vendors)
    n_seeds_per_vendor = s1.groupby("vendor").size()
    n_min = n_seeds_per_vendor.min()
    n_max = n_seeds_per_vendor.max()
    n_mean = n_seeds_per_vendor.mean()

    print(f"\n  Study 1: {n_vendors} vendors, {n_min}-{n_max} seeds per vendor (mean={n_mean:.0f})")
    print(f"  Pairwise comparisons: C({n_vendors},2) = {n_vendors*(n_vendors-1)//2}")
    print(f"  Dimensions: {len(DIMENSIONS)}")
    print(f"  Total tests (before FDR): {n_vendors*(n_vendors-1)//2 * len(DIMENSIONS)}")

    # For each dimension, compute within-group SD (pooled across all vendors)
    print(f"\n  Within-group SD per dimension (used for effect size denominator):")
    within_sds = {}
    for dim in DIMENSIONS:
        all_vals = s1[dim].dropna()
        if len(all_vals) > 0:
            # Pooled within-vendor SD
            vendor_sds = []
            for v in vendors:
                v_vals = s1[s1["vendor"] == v][dim].dropna()
                if len(v_vals) >= 2:
                    vendor_sds.append(np.var(v_vals, ddof=1))
            if vendor_sds:
                pooled_var = np.mean(vendor_sds)
    ...
    print(f"  Assuming n1=n2={n_min} per group (conservative, uses smallest vendor):")
    print(f"  {'d':>6s} {'Power':>8s}  {'Interpretation'}")
    print(f"  {'-'*50}")

    alpha_uncorrected = 0.05
    # With FDR correction over all tests
    n_tests_total = n_vendors * (n_vendors - 1) // 2 * len(DIMENSIONS)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze vendor experiment results")
