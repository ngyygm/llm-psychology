...
\begin{abstract}
A fast-growing literature scores large language models on validated human personality inventories and reports the resulting profiles as if they reflected genuine traits, with downstream proposals to use LLMs as silicon samples for survey simulation. We ask the prior measurement question: when human instruments are administered to LLMs, do they measure anything at all? We administer a 221-item battery built from four validated human inventories (IPIP-NEO-120, SD3, ZKPQ-50-CC, EPQR-A) covering 17 personality domains with 63 reverse-keyed items, to 20 chat-tuned LLMs from 8 vendor families under a Default no-persona instruction and all 16 MBTI persona prompts, with $k{=}5$ samples per cell at $T{=}0.7$ --- 375{,}700 raw API calls in total. We then run six standard psychometric validity checks plus a persona stress test. Every check fails. The failures share a single mechanism: an alignment-induced acquiescence with a directional asymmetry --- a positive forward-minus-reverse agree-rate gap on normal-personality items (IPIP $\Delta{=}{+}0.29$) and a negative gap on dark-trait items (SD3 $\Delta{=}{-}0.33$). Cronbach's $\alpha$ tracks reverse-item density at Spearman $\rho{=}{-}1.0$; exploratory factor analysis universally collapses the five expected Big Five factors into three (Tucker's $\varphi{=}0.993$ across 20 leave-one-model-out reruns); inter-model variance falls to ${<}1\%$ of total sum of squares; and no $(model, persona)$ pair achieves measurement invariance ($r{>}0.8$) against its Default. Persona prompts move profiles by more than one standard deviation per dimension and are $99.7\%$ nearest-centroid recoverable, but cross-scale coherence is only $0.839$ and forward/reverse pair coherence is $0.696$ --- compliance without measurement validity. Calibrated against synthetic baselines, LLM alphas hug the Random and Pure-Acquiescence floor and stay far from a Trait+Acquiescence simulation ($\alpha{>}0.99$): LLMs look like acquiescence with no personality underneath. Any future LLM-personality claim must report internal consistency, factor structure, convergent validity, and measurement invariance before any score can be interpreted.
\end{abstract}
...
\begin{figure}[t]
  ...
  \caption{\textbf{From item-level acquiescence to model-level homogenization.} A four-stage causal chain. Stage~1 (item): alignment training produces an agree-impulse with directional asymmetry --- IPIP normal items $\Delta = +0.29$, SD3 dark items $\Delta = -0.33$. Stage~2 (domain): Cronbach's $\alpha$ tracks reverse-item density at Spearman $\rho = -1.0$. Stage~3 (structural): EFA collapses 5 Big Five factors into 3 universally (Tucker's $\varphi = 0.993$). Stage~4 (model): inter-model variance falls below $1\%$ of total Sum-of-Squares.}
...
\subsection{Default is not neutral}
\label{sec:results-default}

Under leave-one-model-out nearest-centroid classification, 8/20 models project as ISTJ-like under the Default no-persona prompt, with the remaining 12/20 dominated by introverted / judging types; under empirical MBTI-axis projection, 12/20 are ISTJ-like (Appendix~\ref{app:persona}, Figure~\ref{fig:default_persona_classification}). ``No persona'' is therefore a specific persona installed by alignment training --- introverted, structured, cautious, non-confrontational --- not a measurement-neutral baseline.
...
Across the 20 model-level means we test 8 pre-specified construct pairs. Sign-match is $7/8$; the single failure is Extraversion $\times$ ZKPQ Sociability ($r_p = -0.10$, $p = 0.670$), where the Likert and binary-format Sociability scales fail to agree across the 20 models. Strongest convergence: Agreeableness $\times$ Psychopathy ($r_p = -0.81$, $p<0.001$), Conscientiousness $\times$ Psychopathy ($r_p = -0.73$), Agreeableness $\times$ Machiavellianism ($r_p = -0.71$). The full table is in Appendix~\ref{app:dif} (Table~\ref{tab:convergent}). Differential item functioning across Western and Chinese vendor families is non-significant in every domain (all $p > 0.16$, max $|d| = 0.42$).

\subsection{Calibration: LLMs hug the acquiescence floor}
\label{sec:results-calibration}

To distinguish ``trait $+$ noise'' from ``no trait at all'', we simulate three synthetic response strategies on the same IPIP item structure and compute Cronbach's $\alpha$ for each. The Trait+Acquiescence simulation --- which injects a genuine 5-factor latent structure plus an acquiescence offset --- recovers $\alpha > 0.99$ on every domain. Random uniform responding and Pure Acquiescence both yield $\alpha \approx 0$. The observed LLM values sit close to the Random and Pure-Acquiescence baselines on the high-reverse-density domains (Agreeableness $\alpha=0.07$, Openness $\alpha=0.19$) and only approach the human band on the low-reverse-density domains where the acquiescence inflation cannot be detected (Extraversion $\alpha=0.93$, Neuroticism $\alpha=0.81$). Pure acquiescence with no underlying trait is a quantitatively better generative model of the LLM data than ``trait with acquiescence on top.'' Full per-domain values and the calibration figure are in Appendix~\ref{app:dif} (Table~\ref{tab:synthetic}, Figure~\ref{fig:alpha_synthetic_baseline_calibration}).

\subsection{Persona stress test: compliance without validity}
\label{sec:results-persona}

A persona stress test rules out the rescue that personality emerges only when the model is asked to play a character. MBTI prompts move profiles by more than one standard deviation per dimension (mean PSD $= 1.198$, per-model range $0.996$--$1.384$, max PSD $= 1.67$; Appendix~\ref{app:persona}, Figure~\ref{fig:persona_separation_degree}). Nearest-centroid persona classification with leave-one-model-out centroids recovers \textbf{319 / 320} (model, persona) pairs in z-Euclidean distance ($99.7\%$; Figure~\ref{fig:persona_fidelity_confusion_heatmap}); the single failure is DeepSeek-V3.2 ESFP misclassified as ENTP. Likert-only subset reaches $100\%$, cosine $99.7\%$, Mahalanobis $98.4\%$. Empirical target adherence (cosine to LOO centroid) averages $0.838$; theoretical-target adherence (cosine to hand-coded MBTI mapping) averages only $0.557$, indicating that LLMs reliably reproduce an empirical persona response pattern that does not equal the textbook MBTI description.

But persona compliance is not measurement validity. Three independent stress-test statistics fail simultaneously. \textbf{Measurement invariance} (Pearson $r$ between each model's Default item vector and each of its 16 MBTI vectors) averages $r = 0.461$ across the 320 pairs, with $0/320$ above the strong-invariance threshold $r > 0.8$. \textbf{Cross-scale coherence} of persona-induced shifts across instruments that nominally measure the same construct is $0.839$ (95\% CI $[0.815, 0.862]$) --- below the $0.997$ nearest-centroid fidelity, exposing a structural gap. \textbf{Forward/reverse pair coherence} --- the probability that a Fwd/Rev pair moves in opposite raw directions under a persona prompt --- is only $0.696$ overall ($0.737$ EPQR-A, $0.727$ ZKPQ, $0.685$ IPIP, $0.657$ SD3); on roughly one in three Fwd/Rev pairs the persona shift fails the basic directional sanity check. The factorial main-effects model also exposes cross-axis leakage: EPQR Lie loads on J/P ($\beta = 0.78$); SD3 Narcissism loads on E/I ($\beta = 0.94$). The MBTI structure does not separate cleanly inside the LLM response space.

Item-level plasticity (Figure~\ref{fig:item_plasticity_distribution}) shows a sharp dichotomy. The most plastic items shift up to $0.63$ of scale range across persona conditions (EPQR ``Do you sometimes talk about things you know nothing about?'' at $0.632$; ZKPQ ``I lead a busier life than most people'' at $0.608$); the most locked items do not move at all (EPQR ``Do you enjoy hurting people you love?'' at $0.000$; ``Take advantage of others'' at $0.056$). The plasticity floor is the alignment safety filter, not the persona's plausible behaviour. Default-condition within-sample SD ($0.205$ Likert) is also higher than every single MBTI persona ($0.081$--$0.118$): persona prompts \emph{stabilise} responses rather than destabilising them (Appendix~\ref{app:wsc}).
...
A single alignment-induced acquiescence with a directional asymmetry ($\Delta = +0.29$ on normal items; $\Delta = -0.33$ on dark-trait items) explains why every standard psychometric check fails on commercial LLMs: $\alpha$ tracks reverse density at Spearman $\rho = -1.0$; EFA collapses 5 Big Five factors into 3 universally (Tucker's $\varphi = 0.993$); inter-model variance falls below $1\%$; and $0/320$ $(model, persona)$ pairs cross the strong-invariance threshold. Persona prompts are $99.7\%$ recoverable but yield only $0.839$ cross-scale and $0.696$ Fwd/Rev coherence --- compliance, not validity. Calibration places LLM data closer to ``no trait'' than to ``trait $+$ style''. Any LLM-personality claim --- including the silicon-sample programme --- must report internal consistency, factor structure, convergent validity, and measurement invariance before any score is interpreted; multi-temperature, multi-seed designs are required to separate stochastic from putative trait variance. Reading personality off chat-tuned LLMs without auditing the instrument measures the alignment layer, not the model.
