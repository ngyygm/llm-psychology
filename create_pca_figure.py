...
# Model colors (13 models + gray for others)
MODEL_COLORS = {
    "Qwen":      "#E53935",  # red
...
def compute_model_means(results: list) -> list:
    ...
    for (mid, tm), obs in groups.items():
        model = obs[0].get("model", mid.split("/")[0])
...
def plot_pca(model_data: list, output_path: str = "results/vendor_exp/pca_response_styles.pdf"):
    ...
    for model in models:
        model_entries = [(m, i) for i, m in enumerate(valid) if m["model"] == model]
        color = MODEL_COLORS.get(model, "#999999")

        for m, i in model_entries:
            ...
            ax.scatter(x, y, c=color, marker=marker, s=size, alpha=alpha,
                       edgecolors='black', linewidths=0.5, zorder=5, label=model if model_entries.index((m, i)) == 0 else None)
    ...
    # Legend: models only (one per model)
    ...
    custom_markers = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='gray',
               markeredgecolor='black', markersize=8, label='Study 1 (cross-model)'),
...
def plot_pca_by_vendor(model_data: list, output_path: str = "results/vendor_exp/pca_by_vendor.pdf"):
    """Separate PCA plots per model for Study 2 evolution trajectories."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 12), dpi=300)
    fig.patch.set_facecolor('white')

    # Models with Study 2 data
    study2_models = sorted(set(m["model"] for m in model_data if m["study"] == 2))
    top_models = [v for v in ["Qwen", "DeepSeek", "Zhipu"] if v in study2_models]

    for idx, model in enumerate(top_models[:4]):
        ax = axes[idx // 2][idx % 2]
        ax.set_facecolor('white')

        model_data = [m for m in model_data if m["model"] == model]
        if not model_data:
            ax.set_visible(False)
            continue

        X = np.array([[m[d] for d in DIMENSIONS] for m in model_data])
        scaler = StandardScaler()
        X_std = scaler.fit_transform(X)
        pca = PCA(n_components=2)
        X_pca = pca.fit_transform(X_std)
        ...
        subgroups = sorted(set(m.get("subgroup", "") for m in model_data if m["study"] == 2 and m.get("thinking_mode", "chat") != "reasoning"))
        for sg in subgroups:
            sg_data = [m for m in model_data
                       if m.get("subgroup") == sg and m.get("thinking_mode", "chat") != "reasoning"]
            if len(sg_data) > 1:
                for j in range(len(sg_data) - 1):
                    m1 = sg_data[j]
                    m2 = sg_data[j + 1]
                    idx1 = model_data.index(m1)
                    idx2 = model_data.index(m2)
        ...
        ax.set_title(f"{model} — Response Style Trajectory", fontsize=12, fontweight='bold')
        ax.grid(True, alpha=0.2, linestyle='--')
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
    ...
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"Saved model trajectories to {output_path}")
...
def plot_pca_biplot(model_data: list, output_path: str = "results/vendor_exp/pca_biplot.pdf"):
    ...
    # Plot models colored by model
    for model in sorted(set(m["model"] for m in valid)):
        model_idx = [i for i, m in enumerate(valid) if m["model"] == model]
        color = MODEL_COLORS.get(model, "#999999")
        ax.scatter(X_pca[model_idx, 0], X_pca[model_idx, 1],
...
if __name__ == "__main__":
    ...
    for m in model_data:
        print(f"  {m['short_name']:25s} model={m['model']:12s} study={m['study']} "
    ...
    if model_data:
        ...
        # Only plot model trajectories if we have Study 2 data with dimensions
        study2_valid = [m for m in valid_full if m["study"] == 2]
        if len(study2_valid) > 1:
            plot_pca_by_model(model_data)
